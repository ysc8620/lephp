	...
	public function getFields($tableName) {
		...
		if ($result) {
			foreach ( $result as $key => $val ) {
				$info [$val ['Field']] = array ('name' => $val ['Field'], 'type' => $val ['Type'], 'notnull' => ( bool ) ($val ['Null'] == 't' ? 1 : 0), // 't' is 'not null'
'default' => $val ['Default'], 'primary' => (strtolower ( $val ['Key'] ) == 't'), 'autoinc' => (strtolower ( $val ['Extra'] ) == "nextval('{$tableName}_id_seq'::regclass)") );
			}
		}
		return $info;
	}
