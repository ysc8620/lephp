...
+------------------------------------------------------------------------------
 */
class core_driver extends core_db {
	...
	public function connect($config = '', $link_num = 0) {
		if (! isset ( $this->link_id [$link_num] )) {
			...
			$this->link_id [$link_num] = $conn ( $config ['username'], $config ['password'], $config ['database'] ); //modify by wyfeng at 2008.12.19
			

			if (! $this->link_id [$link_num]) {
	...
	public function query($str) {
		...
		if ($this->query_id)
			$this->free ();
		$this->Q ( 1 );
		$this->query_id = oci_parse ( $this->_link_id, $str );
		$this->debug ();
		if (false === oci_execute ( $this->query_id, $this->mode )) {
			$this->error ();
			return false;
		} else {
			return $this->getAll ();
	...
	public function execute($str) {
		...
		if ($this->query_id)
			$this->free ();
		$this->W ( 1 );
		$stmt = oci_parse ( $this->_link_id, $str );
		$this->debug ();
	...
	public function commit() {
		if ($this->transTimes > 0) {
			$result = oci_commit ( $this->_link_id );
			if (! $result) {
	...
	public function rollback() {
		if ($this->transTimes > 0) {
			$result = oci_rollback ( $this->_link_id );
			if (! $result) {
				throw_exception ( $this->error () );
			}
			$this->transTimes = 0;
	...
	private function getAll() {
		//返回数据集
		$result = array ();
		$this->numRows = oci_fetch_all ( $this->query_id, $result, 0, - 1, OCI_FETCHSTATEMENT_BY_ROW );
	...
	public function error($result = true) {
		...
		} elseif (! $this->_link_id) {
			$this->error = oci_error ();
		} else {
			$this->error = oci_error ( $this->_link_id );
		}
