<?php
/**
 * 以range形式散列存储的数据库操作对象
 * 散列参考值不能为空
 * @author aloner
 */
class core_dao_range extends core_dao {
	protected $hash_key = 'id';			//分表规则字段
	protected $row_num_per_table = 1000000;	//每个表中存放数据量
	protected $table_num_per_db = 10;		//每个库中存放表数量
	...
	function __construct(){
		...
		$this -> row_num_per_table = $table -> row_num_per_table;
		$this -> table_num_per_db = $table -> table_num_per_db;
	...
	public function get_rows_by_ids(array $ids){
		//如果哈希字段不是id,则必须提前配置哈希信息
		if($this -> hash_key != 'id'){
			$this -> check_hash_set();
		}else{
			$range = $this -> get_hash($ids);
			$list = array();
			foreach($range as $db=>$tables){
				foreach($tables as $table => $_ids){
					$this -> db($db) -> table($table);
					$list += parent::get_rows_by_ids($_ids);
				}
			}
			return $list;
		}
	...
	function get_hash($keys){
        ...
        $result = array();
        foreach ($keys as $key) {
            $table_index = ceil($key / $this -> row_num_per_table);
            $db_index = ceil($table_index / $this -> table_num_per_db);
            $tb_name = $this -> base_table . "_" . sprintf("%02s", dechex($table_index));
            $db_name = $this -> base_db . "_" . sprintf("%02s", dechex($db_index));
