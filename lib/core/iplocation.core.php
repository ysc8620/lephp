...
class core_iplocation {
	...
	function core_iplocation() {
		$this->fp = 0;
		$this->file_name = CORE_PATH . 'ip.dat';
		if (($this->fp = @fopen ( $this->file_name, 'rb' )) !== false) {
			$this->firstip = $this->getlong ();
			$this->lastip = $this->getlong ();
			$this->totalip = ($this->lastip - $this->firstip) / 7;
			//注册析构函数，使其在程序执行结束时执行
			register_shutdown_function ( array (&$this, '_core_iplocation' ) );
		}
	}
        /**
