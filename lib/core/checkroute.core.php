 ...
 */
class core_checkroute extends core_behavior {
    ...
    public function run(&$return){
        ...
        // 是否开启路由使用
        if(!core_config::get('base.url_router_on')) return $return = false;
        // 路由定义文件优先于config中的配置定义
        $routes = core_config::get('base.url_route_rules');
        // 路由处理
        if(!empty($routes)) {
    ...
    private function parseUrl($url) {
        ...
        if(isset($path)) {
            $var[core_config::get('base.var_action')] = array_pop($path);
            if(!empty($path)) {
                $var[core_config::get('base.var_module')] = array_pop($path);
            }
            if(!empty($path)) {
                $var[core_config::get('base.var_group')]  = array_pop($path);
            }
        }
        return $var;
    }
