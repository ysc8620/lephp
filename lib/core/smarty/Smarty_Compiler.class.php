 ...
 * @package Smarty
 */
class smarty_compiler extends core_smarty_smarty {
    ...
    function _compile_tag($template_tag)
    {
        ...
        switch ($tag_command) {
			...
			case 'list':
                $this->_push_tag('list');
                return $this->_compile_list_start($tag_args);
            case 'listelse':
                $this->_push_tag('listelse');
                return "<?php endforeach; else: ?>";
                break;

            case '/list':
                $_open_tag = $this->_pop_tag('list');
                if ($_open_tag == 'listelse')
                    return "<?php endif; ?>";
                else
	...
	function _compile_list_start($tag_args){
		$attrs = $this->_parse_attrs($tag_args);
		$arg_list = array();
		$output = '<?php ';
	    $list_name = $attrs['name'];
		$output .= "unset(\$this->_lists[$list_name]);\n";

		$item = $attrs['item'];
		if (empty($item)) {
            $this->_syntax_error(" list item 属性未定义", E_USER_ERROR, __FILE__, __LINE__);
        }

		$list_dao = $attrs['dao'];
		if (empty($list_dao)) {
            $this->_syntax_error(" list dao 属性未定义", E_USER_ERROR, __FILE__, __LINE__);
        }
		

		$output .= '$this->_lists[$list_name]';
		$list_where = $attrs['where'];
		$list_page = $attrs['page'];
		$list_order = $attrs['order'];
		
		$output .= '$dao = m('.$list_dao.');';
		if($list_where){
			$output .= ' $dao -> where('.$list_where.'); ';
		}

		if($list_order){
			$output .= ' $dao -> order('.$list_order.');';
		}

		if($list_page){
			$output .= '$dao -> page('.$list_page.');';
		}
		
		$output .= '$data = $dao -> getVoList();';
		$output .=  "\$this->_lists[$list_name] = \$data; ";
		if($list_page){
			$output .=  "\$this->_lists[$list_name]['total'] = \$data['page']->total;";
			$output .=  "\$this->_lists[$list_name]['pager'] = \$data['page']->show();";
		}
		
		$output .= "if (\$data['list']):\n";
		$output .= "foreach(\$data['list'] as $key_part\$this->_tpl_vars[$item]):";
        $output .= "?>";
    ...
    function _compile_smarty_ref(&$indexes)
    {
        ...
        switch ($_ref) {
			...
			case 'yesterday':
				...
				break;
			case 'nextyear':
				$compiled_ref = strtotime('+1 year');
				$_max_index = 1;
				break;
			case 'nextmonth':
				$compiled_ref = strtotime('+1 month');
				$_max_index = 1;
				break;
            case 'foreach':
                array_shift($indexes);
            ...
            default:
                $this->_syntax_error('$smarty.' . $_ref . ' 是个未知标签', E_USER_ERROR, __FILE__, __LINE__);
                break;
        }
    ...
    function _pop_tag($close_tag)
    {
        ...
        if (count($this->_tag_stack)>0) {
			...
			if ($close_tag == 'list' && $_open_tag == 'listelse') {
