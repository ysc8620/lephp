...
class tools_excel_read {
	 ...
	 */
	function tools_excel_read($file='',$store_extended_info=true,$outputEncoding='') {
	...
	function _parse() {
		...
		$code = vl($data,$pos);
		$length = vl($data,$pos+2);
		$version = vl($data,$pos+4);
		$substreamType = vl($data,$pos+6);
		...
		while ($code != SPREADSHEET_EXCEL_READER_TYPE_EOF) {
			switch ($code) {
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
					$indexCode = vl($data,$pos+4);
					if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
						$numchars = vl($data,$pos+6);
						if (ord($data[$pos+8]) == 0){
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FONT:
						$height = vl($data,$pos+4);
						$option = vl($data,$pos+6);
						$color = vl($data,$pos+8);
						$weight = vl($data,$pos+10);
